...
class xmlgooglemaps_googleMapAPI {
	...
	function xmlgooglemaps_googleMapAPI($api_key, $map_type="map") {
		...
		$this->setMapTypeControlButton("USER_MAP_OSM", "hide");
		$this->setMapTypeControlButton("USER_MAP_OCM", "hide");
		$this->setMapTypeControlButton("G_SATELLITE_3D_MAP", "hide");
	...
	function setMapType($type) {
		switch (strtoupper($type)) {
			...
			case 'USER_MAP_OSM':
			case 'OSM':
				$this->map_type = 'USER_MAP_OSM';
				break;
			case 'USER_MAP_OCM':
			case 'OCM':
				$this->map_type = 'USER_MAP_OCM';
				break;				
			case 'G_NORMAL_MAP':
			case 'G_MAP_TYPE':
			case 'MAP':
	...
	function getHeaderScript() {
		...
		$ret .= $this->addLine('<script src="xmlgooglemaps_googleMapsAPI.js" type="text/javascript"></script>');
	...
	function getKMLContentElementOnly($url) {
		...
		if ($this->max_lon > -1000000) {
			if ($this->zoom == 0) {
				$ret .= $this->addLine(sprintf('var zoom = %s.getBoundsZoomLevel(new GLatLngBounds(new GLatLng(%s,%s),new GLatLng(%s,%s)));',$this->map_id,xmlgooglemaps_helper::getJavascriptDecimalNumber($this->min_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->min_lon),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->max_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->max_lon)),4);
				$ret .= $this->addLine(sprintf('%s.setCenter(new GLatLng(%s,%s), zoom + %s);',$this->map_id, xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lon), $this->deltazoom),4);
			} else {
				$ret .= $this->addLine(sprintf('%s.setCenter(new GLatLng(%s,%s), %s);',$this->map_id, xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lon), $this->zoom),4);
			}
		} else {
			if ($this->zoom == 0) {
	...
	function getContentElement() {
		...
		$ret .= $this->addLine(sprintf('%s = new GMap2(document.getElementById("%s"));',$this->map_id,$this->map_id),3);
		if ($this->zoom <= 0) {
			$ret .= $this->addLine(sprintf('var zoom = %s.getBoundsZoomLevel(new GLatLngBounds(new GLatLng(%s,%s),new GLatLng(%s,%s)));',$this->map_id,xmlgooglemaps_helper::getJavascriptDecimalNumber($this->min_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->min_lon),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->max_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->max_lon)),3);
			$ret .= $this->addLine(sprintf('%s.setCenter(new GLatLng(%s,%s), zoom + %s);',$this->map_id, xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lon), $this->deltazoom),3);
		} else {
			$ret .= $this->addLine(sprintf('%s.setCenter(new GLatLng(%s,%s), %s);',$this->map_id, xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lat),xmlgooglemaps_helper::getJavascriptDecimalNumber($this->center_lon), $this->zoom),3);
		}
	...
	function getCommon_MarkersIcons($level=3) {
		...
		foreach($this->markers as $marker) {			
			if (strlen($marker['iconid'])>0) {
				$ret .= $this->addLine(sprintf('var %s_marker%s = new GMarker(new GLatLng(%s,%s), {icon:%s_icon%s} );',$this->map_id, $markerct, xmlgooglemaps_helper::getJavascriptDecimalNumber($marker["lat"]), xmlgooglemaps_helper::getJavascriptDecimalNumber($marker["lon"]), $this->map_id, $marker['iconid']),$level);
			} else {
				$ret .= $this->addLine(sprintf('var %s_marker%s = new GMarker(new GLatLng(%s,%s));',$this->map_id, $markerct, xmlgooglemaps_helper::getJavascriptDecimalNumber($marker["lat"]), xmlgooglemaps_helper::getJavascriptDecimalNumber($marker["lon"])),$level);
			}
	...
	function getCommon_Polylines() {
		foreach($this->polylines as $polyline) {
			...
			for ($i=0; $i<count($pointcol);$i++) {
				...
				$ret .= $this->addLine(sprintf('%s.addOverlay(new GPolyline.fromEncoded({color: "%s", weight: %s, opacity: %s, points: "%s", levels: "%s", zoomFactor: %s, numLevels: %s}));',$this->map_id,$polyline["color"], $polyline["weight"], xmlgooglemaps_helper::getJavascriptDecimalNumber($polyline["opacity"]/100),$encarr[2], $encarr[1], 32, 4),3);
			}
		}
		return $ret;
	}
